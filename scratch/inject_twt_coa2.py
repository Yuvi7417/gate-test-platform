...
def process_html_content(tag):
    ...
    for pre in tag.find_all('pre'):
        ...
        if ol:
            ...
            if lines:
                code_text = '\n'.join(lines)
                parsed_pre = BeautifulSoup(f'<pre style="background-color: #f8f9fa; padding: 15px; border-radius: 5px; border: 1px solid #ddd; overflow-x: auto; font-family: monospace; font-size: 14px; line-height: 1.5;">\n{code_text}\n</pre>', 'html.parser').pre
...
def extract_questions(soup):
    ...
    for div in soup.find_all('div', class_='res_question'):
        ...
        if q_type != 'NAT' and len(options_list) == 0:
...
twt_coa2_js = generate_js(q, "TWT - Computer Organization and Architecture-2", "September 6, 2026")

with open('h:/yuvraj dutt/js/test-registry.src.js', 'a', encoding='utf-8') as f:
    f.write('\n\n' + twt_coa2_js)

print("Injected TWT - Computer Organization and Architecture-2 successfully!")
