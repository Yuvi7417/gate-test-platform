...
async function parse() {
  ...
  for (const qDiv of qDivs) {
    ...
    for (const img of qImages) {
      ...
      if (src && src.startsWith('http')) {
        ...
        // We assume images are already downloaded to images/quiz/wqt-dl8/ from previous run
        $(img).attr('src', `/images/quiz/wqt-dl8/${filename}`);
  ...
  for (const q of questions) {
    ...
    jsContent += `      solution: \`<img src="/images/quiz/wqt-dl8/${qNum}.png" style="max-width: 100%;">\`\n`;
    jsContent += `    },\n`;
    qNum++;
  }
